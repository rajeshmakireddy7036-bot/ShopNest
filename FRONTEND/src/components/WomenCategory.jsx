...
import { Filter, ShoppingBag, ShoppingCart, ChevronDown, X, Heart } from 'lucide-react';
...
const WomenCategory = () => {
    ...
    const fetchProducts = async () => {
        try {
            ...
            const womenOnly = data.filter(p => p.category?.toLowerCase() === 'women');

            if (womenOnly.length > 0) {
                const highestPrice = Math.ceil(Math.max(...womenOnly.map(p => p.price || 0)));
    ...
    const subCategories = ['All', 'Dresses', 'Tops', 'Bottoms', 'Skirts', 'Jackets', 'Watches'];
        ...
        <div className="category-page container">
            <div className="category-hero" style={{
                ...
                <div style={{
                    ...
                    <h1 style={{ fontSize: '4rem', fontWeight: '700', marginBottom: '1rem' }}>Women's Collection</h1>
                    <p style={{ fontSize: '1.2rem', opacity: '0.9' }}>Graceful elegance, empowered style.</p>
                </div>
            </div>
