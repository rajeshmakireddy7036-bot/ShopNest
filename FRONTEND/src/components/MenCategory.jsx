...
import { Filter, ShoppingBag, ShoppingCart, ChevronDown, X, Star, Heart } from 'lucide-react';
...
const MenCategory = () => {
    ...
    const fetchProducts = async () => {
        try {
            ...
            const menOnly = data.filter(p => p.category?.toLowerCase() === 'men');

            if (menOnly.length > 0) {
                const highestPrice = Math.ceil(Math.max(...menOnly.map(p => p.price || 0)));
    ...
    const applyFilters = () => {
        ...
        if (sortBy === 'price-low') {
            ...
            // Assuming higher ID means newer or just use as is
            result.reverse();
        }
    ...
    const subCategories = ['All', 'Shirts', 'Pants', 'Hoodies', 'Jackets', 'Watches'];
        ...
        <div className="category-page container">
            <div className="category-hero" style={{
                ...
                <div style={{
                    ...
                    <h1 style={{ fontSize: '4rem', fontWeight: '700', marginBottom: '1rem' }}>Men's Collection</h1>
                    <p style={{ fontSize: '1.2rem', opacity: '0.9' }}>Timeless style, modern sensibilities.</p>
                </div>
            </div>

            <div className="section-title-wrapper" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <div>
                    <h2 className="section-title" style={{ margin: 0 }}>{selectedSubCategory} Collection</h2>
                ...
                <div style={{ flex: 1 }}>
                    {loading ? (
                        ...
                        <div className="products-grid">
                            {filteredProducts.map(product => (
                                <div key={product.id} className="product-card">
                                    <div className="product-img-container" style={{ position: 'relative', cursor: 'pointer' }} onClick={() => navigate(`/product/${product.id}`)}>
                                        ...
                                        {product.stock === 0 && (
                                            <span style={{ position: 'absolute', top: '10px', left: '10px', backgroundColor: '#fee2e2', color: '#991b1b', padding: '0.2rem 0.6rem', borderRadius: '50px', fontSize: '0.75rem', fontWeight: '600' }}>
                                                Out of Stock
                                            </span>
                                        )}
                                    </div>
                        ...
                        <div style={{ textAlign: 'center', padding: '5rem', background: 'white', borderRadius: '16px', border: '1px solid #edf2f7' }}>
                            ...
                            <p style={{ color: '#6b7280' }}>Try adjusting your filters to find what you're looking for.</p>
